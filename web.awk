BEGIN {
	...
	MAX_WIDTH = 80
	if (ENVIRON["WEB_MAX_WIDTH"] ~ /^[0-9]+$/) {
		if (int(ENVIRON["WEB_MAX_WIDTH"]) < MAX_WIDTH) {
			MAX_WIDTH = int(ENVIRON["WEB_MAX_WIDTH"])
		}
	}
...
function printverse(verse,    word_count, characters_printed) {
	if (ENVIRON["WEB_NOLINEWRAP"] != "" && ENVIRON["WEB_NOLINEWRAP"] != "0") {
		printf("%s\n", verse)
		return
	}
